...
contract Acknowledgement is HasOperators {
  ...
  function getChannelHash(string memory _name)
    public
    view
    returns (bytes32 _channel)
  {
    _channel = _getHash(_name);
    _requireValidChannel(_channel);
  }

  function addChannel(string memory _name)
    public
    onlyAdmin
  {
  ...
  function removeChannel(string memory _name)
    public
    onlyAdmin
  {
    bytes32 _channel = _getHash(_name);
  ...
  function updateValidator(address _validator)
    public
    onlyAdmin
  {
    validator = Validator(_validator);
  ...
  function _getHash(string memory _name)
    internal
    pure
  ...
  function _requireValidChannel(bytes32 _channelHash)
    internal
    view
  {
